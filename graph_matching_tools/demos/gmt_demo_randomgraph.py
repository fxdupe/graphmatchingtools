...
def create_random_graph_method(
        ...
        for graph in graphs:
            for node in graph:
                graph.nodes[node]["weight"] += (
                    np.random.randn(node_data_dim) * node_noise_variance
                )
            for u, v in graph.edges:
                graph.edges[u, v]["weight"] += (
                    np.random.randn(edge_data_dim) * edge_noise_variance
                )
    ...
    if remove_nodes:
        for i_g in range(len(graphs)):
            ...
            for i_n in idx:
                dummies.append(i_n)
                graphs[i_g].nodes[i_n]["weight"] = (
                    np.zeros((node_data_dim, 1)) + 1e9
                )  # Change to impossible values
...
def run_graph_generation(args: argparse.Namespace) -> tuple[float, float]:
    ...
    knode = (
        kutils.create_full_node_affinity_matrix(all_graphs, node_kernel)
        / args.node_number
    )

    truth = pyg.generate_groundtruth(g_sizes, full_size, all_index)
    a_truth = permut.get_permutation_matrix_from_matching(truth, g_sizes)

    x_init = knode * 0.0
    gradient = matching.create_gradient(phi, knode)
    m_res = matching.mkergm(
        gradient,
        g_sizes,
        args.rank,
...
def main() -> None:
    ...
    parser.add_argument(
        "--number_of_runs", help="The number of runs", type=int, default=20
    )
    parser.add_argument(
        "--number_of_graphs", help="The number of graphs", type=int, default=10
    )
    parser.add_argument(
        "--node_number", help="The number of nodes", type=int, default=20
    )
    parser.add_argument(
        "--data_dimension", help="The dimension of the attributes", type=int, default=10
    )
    parser.add_argument(
        "--remove_nodes",
        help="Remove node at random",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--max_node_removed",
        help="The maximal number of node removed",
        type=int,
        default=5,
    )
    parser.add_argument(
        "--add_noise",
        help="Add noise to the data on nodes and edges",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--node_noise",
        help="The variance of the noise on the node data",
        type=float,
        default=1.0,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--proj_method",
        help="The projection method onto the permutation set",
        type=str,
        default="matcheig",
    )

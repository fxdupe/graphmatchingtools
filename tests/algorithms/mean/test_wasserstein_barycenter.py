...
class TestWassersteinBarycenter(TestCase):
    ...
    def test_get_degree_distributions(self):
        ...
        print(degrees)
        self.assertTrue(np.linalg.norm(degrees - np.array([0.25, 0.375, 0.25, 0.125])) < 1e-4)

    def test_get_adjacency_matrix_from_costs_with_valuation(self):
        cost = np.array([[0.37800054, 0.32182417, 0.23277335, 0.17659698],
    ...
    def test_fgw_wasserstein_barycenter(self):
        ...
        graphs = [g1, g2, g3]
        mean_cost, mean_data = mean.fgw_wasserstein_barycenter(graphs, 0.5, 10, 30, 1.0, gamma=0.01)
        print(mean_cost)
        print(mean_data)

        truth_cost = np.array([[0.37800054, 0.32182417, 0.23277335, 0.17659698],
                               [0.32182417, 0.28437325, 0.19532244, 0.15787152],
                               [0.23277335, 0.19532244, 0.14556873, 0.10811781],
                               [0.17659698, 0.15787152, 0.10811781, 0.08939235]])
        truth_data = np.array([[2.35277778, 2.35277778, 2.35277778, 2.35277778], ])
